...
import useProximityConnect from "./useProximityConnect";
...
function Flow(props: FlowProps) {
  ...
  // ----------------------
  const useDagre = false;
  // ----------------------

  const useDragDrop = useDragDropEvent(rfInstance, setNodes);
  const proximityConnect = useProximityConnect(setEdges, nodes, edges);
  const storage = useFlowStorage();

  useEffect(() => {
    if (props.useSelfStorage && nodes.length == 0) {
      storage.restoreFlow(props.defaultFlow, setNodes, setEdges, useDagre);
    }
...
export function ReactFlowWithProvider(props: any) {
  return (
    <ReactFlowProvider>
      <Flow {...props} />
    </ReactFlowProvider>
